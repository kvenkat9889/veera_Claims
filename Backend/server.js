...
async function initializeDatabase() {
  try {
    ...
    if (rowCount === 0) {
      await pool.query(`
        INSERT INTO claims (employee_id, employee_name, title, date, amount, category, description, status, response, attachments)
        VALUES
...
app.get('/api/claims', async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM claims ORDER BY date DESC');
    res.json(rows);
  } catch (err) {
    console.error(err);
...
app.get('/api/claims/employee/:employeeId', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query('SELECT * FROM claims WHERE employee_id = $1 ORDER BY date DESC', [employeeId]);
    res.json(rows);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Internal server error' });
